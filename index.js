import { sleepfrom  } from "./helpers/util.js";
import{SortingAlgorithm} from ".helpers/sortingAlgorithm.js";
...
const generateBtn=documentById('gernerateBtn')
...
const solve =async () =>{
    ...
    for (let i = 0; i < arrayswaps.length; i++) {
        if(swaps[i].firstPosition !== swaps[i].lastPosition){
            await swapBars(barsDivs,swaps[i].firstPosition,swaps[i]
                .lastPosition)
